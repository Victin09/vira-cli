...
class AuthService {
     ...
     * @async
     */
    async oAuth(_token: string, _refreshToken: string, profile: IOauthResponse, next: (e?: Error, v?: User | boolean) => void): Promise<void> {
        try {
            const email = profile.emails
                ? profile.emails
                      .filter((mail) => (mail.hasOwnProperty('verified') && mail.verified) || !mail.hasOwnProperty('verified'))
                      .slice()
                      .shift().value
                : `${profile.name.givenName.toLowerCase()}${profile.name.familyName.toLowerCase()}@externalprovider.com`;
            const iRegistrable = {
                id: profile.id,
