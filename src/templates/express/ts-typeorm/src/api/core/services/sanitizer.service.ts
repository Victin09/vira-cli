...
class SanitizeService {
     ...
     * @description
     */
    private instance: SanitizeService;
    ...
    get(): SanitizeService {
        if (!this.instance) {
            this.instance = new SanitizeService();
        }
    ...
    hasEligibleMember(data: { [key: string]: any }): boolean {
        return (
            ...
            (Array.isArray(data) && [].concat(data).some((obj) => this.implementsWhitelist(obj))) ||
            (isObject(data) && Object.keys(data).some((key) => this.implementsWhitelist(data[key])))
        );
    }
    ...
    private sanitize(entity: IModel): Record<string, unknown> {
        ...
        Object.keys(entity).map((key) => {
            if (entity.whitelist.includes(key) || entity.whitelist.includes(Pluralize(key))) {
                output[key] = this.isSanitizable(entity[key])
                    ? Array.isArray(entity[key])
                        ? entity[key].length > 0
                            ? entity[key].map((e) => this.sanitize(e))
                            : []
                        : this.sanitize(entity[key])
                    : entity[key];
            }
        });
        return output;
...
const sanitizeService = new SanitizeService().get();
